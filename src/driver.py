...
def auto_game() -> None:
    first = True
    while not ToolBox.is_in_home_page():
        sleep(0.1)
    GameController.click_combat_button()
    while True:
        try:
            while not ToolBox.is_in_combat_page():
                sleep(0.1)
                
            GameController.click_combat_menu_button()
            
            while not ToolBox.is_in_combat_menu_page():
                sleep(0.1)
